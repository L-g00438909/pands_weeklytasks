...
def isaweekday():                                     # defines a function called isaweekday(). A function is a block of code which only runs when it is called.  Reference: (https://www.w3schools.com/python/python_functions.asp)
    ...
    return today.weekday() < 5                        # weekday() gives an integer representing days of the week, where monday is 0 and sunday is 6.to check if its a weekday compare the result of weekday to <5. Reference: (https://pynative.com/python-get-the-day-of-week/)         
